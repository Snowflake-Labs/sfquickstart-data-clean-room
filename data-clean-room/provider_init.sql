...
create or replace secure function dcr_samp_provider_db.cleanroom.get_sql_js(template string, parameters variant)
    returns string
    language javascript
as
$$
...
// Although installed, the function is not used unless swapped in the requests() and process_requests() procedures
...
GRANT USAGE ON FUNCTION dcr_samp_provider_db.cleanroom.get_sql_js(string, variant) TO SHARE dcr_samp_app;
